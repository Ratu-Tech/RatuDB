...
public final class JsonTransformer
{
    ...
    public static LinkedBlockingQueue<Map<DecoratedKey,Row>> rows = new LinkedBlockingQueue<>();
    ...
    private void serializePartition(UnfilteredRowIterator partition)
    {
        try
        {
            ...
            while (partition.hasNext())
            {
                ...
                try {
                    Map<DecoratedKey,Row> rowMap=new HashMap<>();
                    rowMap.put(partition.partitionKey(), (Row)unfiltered);
                    rows.put(rowMap);
