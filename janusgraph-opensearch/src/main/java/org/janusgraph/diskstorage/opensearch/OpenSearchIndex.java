...
public class OpenSearchIndex implements IndexProvider {
    ...
    public static final ConfigOption<Integer> SOCKET_TIMEOUT =
        new ConfigOption<>(OPENSEARCH_NS, "socket-timeout",
            ...
            Integer.class, RestClientBuilder.DEFAULT_RESPONSE_TIMEOUT_MILLIS);
                ...
                for (final Map.Entry<String, IndexMutation> entry : stores.getValue().entrySet()) {
                    ...
                    if (mutation.hasDeletions()) {
                        if (mutation.isDeleted()) {
                            ...
                            Map<String,Object> doc = compat.prepareStoredScript(parameterizedDeletionScriptId, params).build();
                    ...
                    if (mutation.hasAdditions()) {
                        if (mutation.isNew()) { //Index
                            ...
                            final Map<String,Object> upsert;
                            if (!mutation.hasDeletions()) {
                            ...
                            if (!params.isEmpty()) {
                                ImmutableMap.Builder<String,Object> builder = compat.prepareStoredScript(parameterizedAdditionScriptId, params);
                                requestByStore.add(OpenSearchMutation.createUpdateRequest(indexStoreName, storeName,
    ...
    public Map<String,Object> getFilter(Condition<?> condition, KeyInformation.StoreRetriever information) {
        if (condition instanceof PredicateCondition) {
            ...
            final List<Map<String,Object>> queries = IterablesUtil.stream(condition.getChildren())
